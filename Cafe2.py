...
with open('menu2_data.json', 'r' , encoding='utf-8') as file:
...
Tops.configure(background='#AED6F1')
fMainL.configure(background='#333333')
fMainR.configure(background='#333333')

lblInfo = Label(Tops,font=('Adobe Hebrew',60,'bold'),text = "Cafe Shop",bg="#AED6F1",bd=10)
...
def Paywithcard():
    ...
    user['history'].append({'amount': totalPrice, 'datetime': datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'type': 'pay', 'service': 'store2', 'point': pointCal})
